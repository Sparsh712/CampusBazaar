...
export default function SellModal({ onClose, onList, accountAddress }) {
    ...
    const handleSubmit = () => {
        ...
        onList({ ...form, price: parseFloat(form.price), id: Date.now(), seller: accountAddress, date: new Date().toISOString(), rating: 0, ratingCount: 0 });
        onClose();
    };
            ...
            <div style={{ background: "#0f172a", border: "1px solid #1f2937", borderRadius: 20, padding: 32, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", animation: "scaleIn .25s ease" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 12 }}>
                        <button onClick={onClose} style={{ flex: 1, padding: "12px 0", borderRadius: 10, border: "1px solid #1f2937", background: "none", color: "#9ca3af", cursor: "pointer", fontWeight: 600 }}>Cancel</button>
                        <button onClick={handleSubmit} style={{ flex: 2, padding: "12px 0", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 15 }}>List Item</button>
                    </div>
                </div>
            </div>
        </div>
