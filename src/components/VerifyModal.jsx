...
const VALID_DOMAINS = ["@dtu.ac.in", "@iitd.ac.in", "@iitb.ac.in", "@du.ac.in", "@nsut.ac.in", "@iiitd.ac.in", "@bits-pilani.ac.in", "@nith.ac.in"];
...
export default function VerifyModal({ onClose, onVerify }) {
    ...
    const handleSendCode = () => {
        ...
        if (!VALID_DOMAINS.some(d => domain.toLowerCase() === d)) {
            setError(`Only university emails accepted: ${VALID_DOMAINS.slice(0, 4).join(", ")}...`);
            return;
        }
        setStep("code");
    };
                        ...
                        {step === "input" ? (
                            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                                <input style={inputStyle} type="email" value={email} onChange={e => setEmail(e.target.value)}
                                    placeholder="you@dtu.ac.in" onKeyDown={e => e.key === "Enter" && handleSendCode()} />
